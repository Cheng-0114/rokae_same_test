...
namespace rokae_hardware    //用override虚函数对基类SystemInterface的成员函数进行复写，否则子类RokaeHardwareInterface中的方法可能无法被ROS2中接口SystemInterface调用（）
{
    ...
    public:
        ...
        {
            if (auto *base = robotBase()) {
                base->setMotionControlMode(rokae::MotionControlMode::NrtCommand, ec);
            }
        }
    ...
    private:
        rokae::BaseRobot * robotBase() noexcept;
        bool stopRtMove() noexcept;
        bool startRtJointPosition() noexcept;
        bool startRtTorque() noexcept;
        bool configureServoJoint() noexcept;
        void sendJointPositionCommand(const rokae::JointPosition &jcmd);
        bool recoverRtSession(bool restart_position_mode) noexcept;
    ...
    public:
        ...
        std::shared_ptr<rokae::StandardRobot> industrial_robot_;
        std::string xMate_type_;
        std::string robot_class_;
        bool use_industrial_robot_ = false;
        ...
        std::shared_ptr<rokae::RtMotionControlIndustrial<6>> industrial_rci_;

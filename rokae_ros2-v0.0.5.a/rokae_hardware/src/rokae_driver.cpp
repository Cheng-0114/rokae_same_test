...
public:
    ...
    void connectToRobot(const std::string &remote_ip, const std::string &local_ip)
    {
        if (industrial_) {
            standard_.connectToRobot(remote_ip, local_ip);
        } else {
            cobot_.connectToRobot(remote_ip, local_ip);
        }
    }

    rokae::Info robotInfo(std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.robotInfo(out_ec) : cobot_.robotInfo(out_ec);
    }

    std::string sdkVersion()
    {
        return industrial_ ? standard_.sdkVersion() : cobot_.sdkVersion();
    }

    OperationState operationState(std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.operationState(out_ec) : cobot_.operationState(out_ec);
    }

    void pauseProject(std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.pauseProject(out_ec);
        } else {
            cobot_.pauseProject(out_ec);
        }
    }

    void stop(std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.stop(out_ec);
        } else {
            cobot_.stop(out_ec);
        }
    }
    ...
    bool withRtController(Func func)
    {
        if (industrial_) {
            auto rt = standard_.getRtMotionController().lock();
            if (!rt) {
                return false;
            }
            func(rt);
        ...
        auto rt = cobot_.getRtMotionController().lock();
        if (!rt) {
            return false;
        }
        func(rt);
    ...
    void setOperateMode(OperateMode mode, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setOperateMode(mode, out_ec);
        } else {
            cobot_.setOperateMode(mode, out_ec);
        }
    }

    void setPowerState(bool on, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setPowerState(on, out_ec);
        } else {
            cobot_.setPowerState(on, out_ec);
        }
    }

    void setMotionControlMode(MotionControlMode mode, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setMotionControlMode(mode, out_ec);
        } else {
            cobot_.setMotionControlMode(mode, out_ec);
        }
    }

    void enableDrag(DragParameter::Space space, DragParameter::Type type, std::error_code &out_ec)
    {
        if (industrial_) {
            throw std::runtime_error("Drag control is not supported on industrial StandardRobot");
        }
        cobot_.enableDrag(space, type, out_ec);
    }

    void disableDrag(std::error_code &out_ec)
    {
        if (industrial_) {
            throw std::runtime_error("Drag control is not supported on industrial StandardRobot");
        }
        cobot_.disableDrag(out_ec);
    }

    auto model()
    {
        return industrial_ ? standard_.model() : cobot_.model();
    }

    void startJog(JogOpt::Space space, double rate, double step, unsigned index, bool direction, std::error_code &out_ec)
    {
        if (industrial_) {
            throw std::runtime_error("Jog control is not supported on industrial StandardRobot");
        }
        cobot_.startJog(space, rate, step, index, direction, out_ec);
    }

    bool getDO(unsigned int board, unsigned int port, std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.getDO(board, port, out_ec) : cobot_.getDO(board, port, out_ec);
    }

    void setDO(unsigned int board, unsigned int port, bool state, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setDO(board, port, state, out_ec);
        } else {
            cobot_.setDO(board, port, state, out_ec);
        }
    }

    void setSimulationMode(bool on, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setSimulationMode(on, out_ec);
        } else {
            cobot_.setSimulationMode(on, out_ec);
        }
    }

    void setDI(unsigned int board, unsigned int port, bool state, std::error_code &out_ec) noexcept
    {
        if (industrial_) {
            standard_.setDI(board, port, state, out_ec);
        } else {
            cobot_.setDI(board, port, state, out_ec);
        }
    }

    bool getDI(unsigned int board, unsigned int port, std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.getDI(board, port, out_ec) : cobot_.getDI(board, port, out_ec);
    }

    template<typename... Args>
    void readRegister(Args &&... args)
    {
        if (industrial_) {
            standard_.readRegister(std::forward<Args>(args)...);
        } else {
            cobot_.readRegister(std::forward<Args>(args)...);
        }
    }

    template<typename... Args>
    void writeRegister(Args &&... args)
    {
        if (industrial_) {
            standard_.writeRegister(std::forward<Args>(args)...);
        } else {
            cobot_.writeRegister(std::forward<Args>(args)...);
        }
    }

    std::array<double, 6> jointPos(std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.jointPos(out_ec) : cobot_.jointPos(out_ec);
    }

    std::array<double, 6> jointVel(std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.jointVel(out_ec) : cobot_.jointVel(out_ec);
    }

    std::array<double, 6> jointTorque(std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.jointTorque(out_ec) : cobot_.jointTorque(out_ec);
    }

    std::array<double, 6> posture(CoordinateType type, std::error_code &out_ec) noexcept
    {
        return industrial_ ? standard_.posture(type, out_ec) : cobot_.posture(type, out_ec);
    }

private:
    rokae::xMateRobot cobot_;
    rokae::StandardRobot standard_;
};
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    try {
        ...
        try {
            robot.withRtController([&](auto rt_con) {
                try {
    ...
    std::shared_ptr<rokae_msgs::srv::MoveJ::Response> response)
{
    try {
        ...
        robot.setPowerState(true, ec);
        if (!robot.withRtController([&](auto rtCon) {
...
int main(int argc , char** argv){
    ...
    node->declare_parameter("robot_class", "collaborative");
    node->declare_parameter("xMate_type", "");
    std::string robot_ip = node->get_parameter("robot_ip").as_string();
    std::string local_ip = node->get_parameter("local_ip").as_string();
    const std::string robot_class = node->get_parameter("robot_class").as_string();
    const std::string xmate_type = node->get_parameter("xMate_type").as_string();
    robot.industrial_ = (
        robot_class == "standard" || xmate_type == "XB7s" || xmate_type == "XB7h" ||
        xmate_type == "NB25s" || xmate_type == "NB25h" || xmate_type == "NB12s" ||
        xmate_type == "NB12h" ||
        xmate_type == "EB4" || xmate_type == "NB4" || xmate_type == "XB4s" || xmate_type == "XB4h");

    // 连接到机器人
    try {
        robot.connectToRobot(robot_ip ,local_ip);
        RCLCPP_INFO(
            rclcpp::get_logger("rokae_driver"),
            "Connected (%s) robot_class=%s xMate_type=%s",
            robot.industrial_ ? "StandardRobot" : "xMateRobot",
            robot_class.c_str(), xmate_type.c_str());
    } catch (const std::exception &e) {

...
public:
    ...
    : Node("movej_demo_node")
    {
        ...
        auto_execute_ = this->declare_parameter("auto_execute", false);
    }
    ...
    bool autoExecute() const { return auto_execute_; }
...
int main(int argc, char** argv)
{
    ...
    // 5) Optional demo motion (disabled unless auto_execute:=true)
    if (!node->autoExecute()) {
        RCLCPP_INFO(
            node->get_logger(),
            "movej idle (auto_execute=false). Use RViz Plan & Execute or launch with enable_movej:=true.");
        rclcpp::shutdown();
        spinner.join();
        return 0;
    }
